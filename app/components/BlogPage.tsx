      ...
      <div className="relative z-10 container mx-auto px-6 py-12">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <p className="text-xl text-gray-300 mb-12 text-center max-w-3xl mx-auto">
            3D技術の最新トレンド、業界のニュース、そして私たちのサービスに関する洞察を、定期的にお届けします。
            ここでは、3D CAD、3Dスキャン、そして技術研修に関する有益な情報を見つけることができます。
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {currentPosts.map((post, index) => (
            ...
            >
              <article className="bg-gray-800 rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition duration-300 h-full flex flex-col">
                {post.coverImage && (
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <Link
                        href={`/blog/${post.id}`}
                        className="inline-flex items-center text-blue-400 hover:text-blue-300 transition duration-300 font-semibold hover:scale-105 transform"
                      >
                        続きを読む
                        <svg
          ...
          {Array.from({ length: Math.ceil(blogPosts.length / postsPerPage) }, (_, i) => (
            <motion.button
              ...
              className={`mx-1 px-4 py-2 rounded-full text-base font-semibold transition-all duration-300 ${
                currentPage === i + 1 ? "bg-blue-600 text-white" : "bg-gray-700 text-white hover:bg-gray-600"
              }`}
              ...
              <div className="fixed inset-0 flex items-center justify-center p-4 overflow-y-auto">
                <Dialog.Panel>
                  <motion.div
                    ...
                    className="mx-auto max-w-2xl w-full max-h-[80vh] overflow-y-auto rounded-2xl bg-gray-800 p-6 shadow-xl relative"
                  >
                    <button
